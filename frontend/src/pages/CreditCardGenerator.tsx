          ...
          <div className="flex-1 flex flex-col justify-center gap-8 min-w-[260px]">
            <div>
              <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight text-gray-900 mb-4 font-sans drop-shadow-xl flex items-center gap-3">
                <span className="text-4xl md:text-5xl">💳</span> {t('creditCardGenerator')}
              </h1>
              <p className="text-lg md:text-xl text-gray-700 font-medium mb-6">
                {t('creditCardGeneratorSubtitle')}
              </p>
            </div>
            ...
            <div>
              ...
              {generatedCard ? (
                <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 border border-orange-100 shadow-lg flex flex-col gap-4 animate-fade-in">
                  {/* 信用卡显示 */}
                  <div className="bg-gradient-to-r from-orange-400 to-orange-600 rounded-lg p-6 text-white mb-6 shadow-lg flex flex-col gap-2">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="text-sm opacity-75">Card Type</p>
                        <p className="font-semibold text-lg">{generatedCard.card_type}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm opacity-75">CVV</p>
                        <p className="font-semibold text-lg">{generatedCard.cvv}</p>
                      </div>
                    </div>
                    <div className="mb-2">
                      <p className="text-sm opacity-75 mb-1">Card Number</p>
                      <p className="text-2xl font-mono tracking-wider select-all">
                    ...
                    <div className="flex justify-between">
                      <div>
                        ...
                        <p className="font-semibold text-lg">{generatedCard.expiry_month}/{generatedCard.expiry_year}</p>
                      </div>
                    </div>
                  </div>
                  {/* 复制按钮 */}
                  <div className="space-y-3">
                    ...
                    <button
                      onClick={copyAllData}
                      className="w-full bg-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-4 rounded-xl transition-all duration-200 shadow hover:shadow-lg focus:ring-4 focus:ring-orange-400/40"
                    >
